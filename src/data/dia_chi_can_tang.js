export const dia_chi_diem_co_ban = {
  "tý": {
    "can_tang": {
      "quý": 100
    }
  },
  "sửu": {
    "can_tang": {
      "kỷ": 60,
      "quý": 30,
      "tân": 10
    }
  },
  "dần": {
    "can_tang": {
      "giáp": 60,
      "bính": 30,
      "mậu": 10
    }
  },
  "mão": {
    "can_tang": {
      "ất": 100
    }
  },
  "thìn": {
    "can_tang": {
      "mậu": 60,
      "ất": 30,
      "quý": 10
    }
  },
  "tỵ": {
    "can_tang": {
      "bính": 60,
      "mậu": 30,
      "canh": 10
    }
  },
  "ngọ": {
    "can_tang": {
      "đinh": 70,
      "kỷ": 30
    }
  },
  "mùi": {
    "can_tang": {
      "kỷ": 60,
      "đinh": 30,
      "ất": 10
    }
  },
  "thân": {
    "can_tang": {
      "canh": 60,
      "nhâm": 30,
      "mậu": 10
    }
  },
  "dậu": {
    "can_tang": {
      "tân": 100
    }
  },
  "tuất": {
    "can_tang": {
      "mậu": 60,
      "tân": 30,
      "đinh": 10
    }
  },
  "hợi": {
    "can_tang": {
      "nhâm": 70,
      "giáp": 30
    }
  }
}


export const thien_can_dai_van = {
  "giáp": {
      "giáp": 100,
      "ất": 50,
      "bính": 30,
      "đinh": 20,
      "mậu": -50,
      "kỷ": -30,
      "canh": -20,
      "tân": 0,
      "nhâm": -20,
      "quý": -30
  },
  "ất": {
      "giáp": 50,
      "ất": 100,
      "bính": 20,
      "đinh": 30,
      "mậu": -30,
      "kỷ": -50,
      "canh": 0,
      "tân": -20,
      "nhâm": -30,
      "quý": -20
  },
  "bính": {
      "giáp": -20,
      "ất": -30,
      "bính": 100,
      "đinh": 50,
      "mậu": 20,
      "kỷ": 30,
      "canh": -50,
      "tân": -30,
      "nhâm": -20,
      "quý": 0
  },
  "đinh": {
      "giáp": -30,
      "ất": -20,
      "bính": 50,
      "đinh": 100,
      "mậu": 30,
      "kỷ": 20,
      "canh": -30,
      "tân": -50,
      "nhâm": 0,
      "quý": -20
  },
  "mậu": {
      "giáp": 0,
      "ất": 0,
      "bính": -20,
      "đinh": -30,
      "mậu": 100,
      "kỷ": 50,
      "canh": 20,
      "tân": 30,
      "nhâm": -50,
      "quý": -30
  },
  "kỷ": {
      "giáp": 0,
      "ất": 0,
      "bính": -30,
      "đinh": -20,
      "mậu": 50,
      "kỷ": 100,
      "canh": 30,
      "tân": 20,
      "nhâm": -30,
      "quý": -50
  },
  "canh": {
      "giáp": -50,
      "ất": -30,
      "bính": 0,
      "đinh": 0,
      "mậu": -20,
      "kỷ": -30,
      "canh": 100,
      "tân": 50,
      "nhâm": 20,
      "quý": 30
  },
  "tân": {
      "giáp": -30,
      "ất": -50,
      "bính": 0,
      "đinh": 0,
      "mậu": -30,
      "kỷ": -20,
      "canh": 50,
      "tân": 100,
      "nhâm": 30,
      "quý": 20
  },
  "nhâm": {
      "giáp": 20,
      "ất": 30,
      "bính": -50,
      "đinh": -30,
      "mậu": 0,
      "kỷ": 0,
      "canh": -20,
      "tân": -30,
      "nhâm": 100,
      "quý": 50
  },
  "quý": {
      "giáp": 30,
      "ất": 20,
      "bính": -30,
      "đinh": -50,
      "mậu": 0,
      "kỷ": 0,
      "canh": -30,
      "tân": -20,
      "nhâm": 50,
      "quý": 100
  }
}


export const dia_chi_dai_van = {
  "tý": {
    "sửu": {
      "kỷ": 0,
      "quý": 100,
      "tân": -20
    },
    "mão": {
      "ất": 20
    },
    "thìn": {
      "mậu": -30,
      "ất": 20,
      "quý": 100
    },
    "ngọ": {
      "đinh": -50,
      "kỷ": 0
    },
    "mùi": {
      "kỷ": 0,
      "đinh": -50,
      "ất": 20
    },
    "thân": {
      "canh": -30,
      "nhâm": 50,
      "mậu": -30
    }
  },
  "sửu": {
    "tý": {
      "quý": -30
    },
    "tỵ": {
      "bính": -40,
      "mậu": 40,
      "canh": 30
    },
    "ngọ": {
      "đinh": -30,
      "kỷ": 70
    },
    "mùi": {
      "kỷ": 100,
      "đinh": -30,
      "ất": -30
    },
    "dậu": {
      "tân": 20
    },
    "tuất": {
      "mậu": 40,
      "tân": 30,
      "đinh": -70
    }
  },
  "dần": {
    "tỵ": {
      "bính": 0,
      "mậu": -30,
      "canh": -60
    },
    "ngọ": {
      "đinh": 40,
      "kỷ": -20
    },
    "thân": {
      "canh": -40,
      "nhâm": -40,
      "mậu": -30
    },
    "tuất": {
      "mậu": -40,
      "tân": -30,
      "đinh": 80
    },
    "hợi": {
      "nhâm": -40,
      "giáp": 80
    }
  },
  "mão": {
    "tý": {
      "quý": -20
    },
    "thìn": {
      "mậu": -30,
      "ất": 100,
      "quý": -20
    },
    "mùi": {
      "kỷ": -50,
      "đinh": 20,
      "ất": 100
    },
    "dậu": {
      "tân": -20
    },
    "tuất": {
      "mậu": -30,
      "tân": -20,
      "đinh": 20
    }
  },
  "thìn": {
    "tý": {
      "quý": -40
    },
    "mão": {
      "ất": 30
    },
    "thìn": {
      "mậu": 80,
      "ất": 100,
      "quý": -50
    },
    "thân": {
      "canh": 20,
      "nhâm": -80,
      "mậu": 70
    },
    "dậu": {
      "tân": 20
    },
    "tuất": {
      "mậu": 80,
      "tân": 20,
      "đinh": -10
    }
  },
  "tỵ": {
    "sửu": {
      "kỷ": 50,
      "quý": -30,
      "tân": -20
    },
    "dần": {
      "giáp": -20,
      "bính": 80,
      "mậu": 100
    },
    "thân": {
      "canh": -40,
      "nhâm": -70,
      "mậu": 100
    },
    "dậu": {
      "tân": -20
    },
    "hợi": {
      "nhâm": -40,
      "giáp": -20
    }
  },
  "ngọ": {
    "tý": {
      "quý": -40
    },
    "sửu": {
      "kỷ": 70,
      "quý": -70,
      "tân": -30
    },
    "dần": {
      "giáp": -30,
      "bính": 30,
      "mậu": 80
    },
    "ngọ": {
      "đinh": 80,
      "kỷ": 100
    },
    "mùi": {
      "kỷ": 70,
      "đinh": 80,
      "ất": -20
    },
    "tuất": {
      "mậu": 50,
      "tân": -40,
      "đinh": 80
    }
  },
  "mùi": {
    "tý": {
      "quý": -70
    },
    "sửu": {
      "kỷ": 80,
      "quý": -70,
      "tân": -30
    },
    "mão": {
      "ất": -20
    },
    "ngọ": {
      "đinh": 70,
      "kỷ": 80
    },
    "tuất": {
      "mậu": 60,
      "tân": -20,
      "đinh": 80
    },
    "hợi": {
      "nhâm": -40,
      "giáp": -30
    }
  },
  "thân": {
    "tý": {
      "quý": 50
    },
    "dần": {
      "giáp": -40,
      "bính": -50,
      "mậu": -20
    },
    "thìn": {
      "mậu": -20,
      "ất": 0,
      "quý": 80
    },
    "tỵ": {
      "bính": -30,
      "mậu": -20,
      "canh": 80
    },
    "hợi": {
      "nhâm": 70,
      "giáp": -30
    }
  },
  "dậu": {
    "sửu": {
      "kỷ": -20,
      "quý": 20,
      "tân": 100
    },
    "mão": {
      "ất": -50
    },
    "thìn": {
      "mậu": -30,
      "ất": -50,
      "quý": 20
    },
    "tỵ": {
      "bính": -20,
      "mậu": -30,
      "canh": 50
    },
    "dậu": {
      "tân": 100
    },
    "tuất": {
      "mậu": -30,
      "tân": 100,
      "đinh": 0
    }
  },
  "tuất": {
    "sửu": {
      "kỷ": 40,
      "quý": -20,
      "tân": 100
    },
    "dần": {
      "giáp": -10,
      "bính": -30,
      "mậu": 70
    },
    "mão": {
      "ất": -20
    },
    "thìn": {
      "mậu": 80,
      "ất": -50,
      "quý": -10
    },
    "ngọ": {
      "đinh": -30,
      "kỷ": 30
    },
    "mùi": {
      "kỷ": 40,
      "đinh": -30,
      "ất": -50
    },
    "dậu": {
      "tân": 80
    }
  },
  "hợi": {
    "dần": {
      "giáp": 80,
      "bính": -30,
      "mậu": -50
    },
    "mão": {
      "ất": 50
    },
    "tỵ": {
      "bính": -40,
      "mậu": -50,
      "canh": -40
    },
    "mùi": {
      "kỷ": -20,
      "đinh": 0,
      "ất": 80
    },
    "thân": {
      "canh": -30,
      "nhâm": 100,
      "mậu": -50
    },
    "hợi": {
      "nhâm": 90,
      "giáp": 100
    }
  }
}

export const nguyet_lenh = {
  "nguyet_lenh_1": {
    "description": "Sinh vào tháng Dần, Mão và 12 ngày đầu tháng Thìn.",
    "gia_giam_tri_so": {
      "giáp": 100,
      "ất": 100,
      "bính": 30,
      "đinh": 30,
      "mậu": -50,
      "kỷ": -50,
      "canh": -30,
      "tân": -30,
      "nhâm": -20,
      "quý": -20
    }
  },
  "nguyet_lenh_2": {
    "description": "Sinh vào tháng Tỵ, Ngọ và 12 ngày đầu tháng Mùi.",
    "gia_giam_tri_so": {
      "giáp": -20,
      "ất": -20,
      "bính": 100,
      "đinh": 100,
      "mậu": 30,
      "kỷ": 30,
      "canh": -50,
      "tân": -50,
      "nhâm": -30,
      "quý": -30
    }
  },
  "nguyet_lenh_3": {
    "description": "Sinh vào tháng Thân, Dậu và 12 ngày đầu tháng Tuất.",
    "gia_giam_tri_so": {
      "giáp": -50,
      "ất": -50,
      "bính": -30,
      "đinh": -30,
      "mậu": -20,
      "kỷ": -20,
      "canh": 100,
      "tân": 100,
      "nhâm": 30,
      "quý": 30
    }
  },
  "nguyet_lenh_4": {
    "description": "Sinh vào tháng Hợi, Tý và 12 ngày đầu tháng Sửu.",
    "gia_giam_tri_so": {
      "giáp": 30,
      "ất": 30,
      "bính": -50,
      "đinh": -50,
      "mậu": -30,
      "kỷ": -30,
      "canh": -20,
      "tân": -20,
      "nhâm": 100,
      "quý": 100
    }
  },
  "nguyet_lenh_5": {
    "description": "Sinh vào 18 ngày cuối các tháng Thìn, Mùi, Tuất, Sửu.",
    "gia_giam_tri_so": {
      "giáp": -30,
      "ất": -30,
      "bính": -20,
      "đinh": -20,
      "mậu": 100,
      "kỷ": 100,
      "canh": 30,
      "tân": 30,
      "nhâm": -50,
      "quý": -50
    }
  }
}


export const tri_so_can_chi = {
  "giáp tý": {
    "thiên_can": 20,
    "can_tàng": { "quý": -20 }
  },
  "ất sửu": {
    "thiên_can": -30,
    "can_tàng": { "kỷ": -50 }
  },
  "bính dần": {
    "thiên_can": 30,
    "can_tàng": { "giáp": -30 }
  },
  "đinh mão": {
    "thiên_can": 20,
    "can_tàng": { "ất": -20 }
  },
  "mậu thìn": {
    "thiên_can": 50,
    "can_tàng": { "mậu": 50 }
  },
  "kỷ tỵ": {
    "thiên_can": 30,
    "can_tàng": { "bính": -30 }
  },
  "canh ngọ": {
    "thiên_can": -50,
    "can_tàng": { "đinh": -30 }
  },
  "tân mùi": {
    "thiên_can": 20,
    "can_tàng": { "kỷ": -20 }
  },
  "nhâm thân": {
    "thiên_can": 30,
    "can_tàng": { "canh": -30 }
  },
  "quý dậu": {
    "thiên_can": 20,
    "can_tàng": { "tân": -20 }
  },
  "giáp tuất": {
    "thiên_can": -30,
    "can_tàng": { "mậu": -50 }
  },
  "ất hợi": {
    "thiên_can": 30,
    "can_tàng": { "nhâm": -30 }
  },
  "bính tý": {
    "thiên_can": -50,
    "can_tàng": { "quý": -25 }
  },
  "đinh sửu": {
    "thiên_can": -30,
    "can_tàng": { "kỷ": 30 }
  },
  "mậu dần": {
    "thiên_can": -50,
    "can_tàng": { "giáp": -30 }
  },
  "kỷ mão": {
    "thiên_can": -50,
    "can_tàng": { "ất": -25 }
  },
  "canh thìn": {
    "thiên_can": 20,
    "can_tàng": { "mậu": -20 }
  },
  "tân tỵ": {
    "thiên_can": -50,
    "can_tàng": { "bính": -30 }
  },
  "nhâm ngọ": {
    "thiên_can": -30,
    "can_tàng": { "đinh": -45 }
  },
  "quý mùi": {
    "thiên_can": -50,
    "can_tàng": { "kỷ": -25 }
  },
  "giáp thân": {
    "thiên_can": -50,
    "can_tàng": { "canh": -30 }
  },
  "ất dậu": {
    "thiên_can": -50,
    "can_tàng": { "tân": -25 }
  },
  "bính tuất": {
    "thiên_can": -30,
    "can_tàng": { "mậu": 30 }
  },
  "đinh hợi": {
    "thiên_can": -50,
    "can_tàng": { "nhâm": -30 }
  },
  "mậu tý": {
    "thiên_can": -30,
    "can_tàng": { "quý": -45 }
  },
  "kỷ sửu": {
    "thiên_can": 50,
    "can_tàng": { "kỷ": 50 }
  },
  "canh dần": {
    "thiên_can": -30,
    "can_tàng": { "giáp": -45 }
  },
  "tân mão": {
    "thiên_can": -30,
    "can_tàng": { "ất": -45 }
  },
  "nhâm thìn": {
    "thiên_can": -40,
    "can_tàng": { "mậu": -30 }
  },
  "quý tỵ": {
    "thiên_can": -30,
    "can_tàng": { "bính": -45 }
  },
  "giáp ngọ": {
    "thiên_can": -30,
    "can_tàng": { "đinh": 30 }
  },
  "ất mùi": {
    "thiên_can": -30,
    "can_tàng": { "kỷ": -50 }
  },
  "bính thân": {
    "thiên_can": -30,
    "can_tàng": { "canh": -45 }
  },
  "đinh dậu": {
    "thiên_can": -30,
    "can_tàng": { "tân": -45 }
  },
  "mậu tuất": {
    "thiên_can": 50,
    "can_tàng": { "mậu": 50 }
  },
  "kỷ hợi": {
    "thiên_can": -30,
    "can_tàng": { "nhâm": -45 }
  },
  "canh tý": {
    "thiên_can": -30,
    "can_tàng": { "quý": 30 }
  },
  "tân sửu": {
    "thiên_can": 30,
    "can_tàng": { "kỷ": -30 }
  },
  "nhâm dần": {
    "thiên_can": -30,
    "can_tàng": { "giáp": 30 }
  },
  "quý mão": {
    "thiên_can": -30,
    "can_tàng": { "ất": 30 }
  },
  "giáp thìn": {
    "thiên_can": 0,
    "can_tàng": { "mậu": -70 }
  },
  "ất tỵ": {
    "thiên_can": -30,
    "can_tàng": { "bính": 30 }
  },
  "bính ngọ": {
    "thiên_can": 50,
    "can_tàng": { "đinh": 50 }
  },
  "đinh mùi": {
    "thiên_can": -30,
    "can_tàng": { "kỷ": 30 }
  },
  "mậu thân": {
    "thiên_can": -30,
    "can_tàng": { "canh": 30 }
  },
  "kỷ dậu": {
    "thiên_can": -30,
    "can_tàng": { "tân": 30 }
  },
  "canh tuất": {
    "thiên_can": 30,
    "can_tàng": { "mậu": -30 }
  },
  "tân hợi": {
    "thiên_can": -30,
    "can_tàng": { "nhâm": 30 }
  },
  "nhâm tý": {
    "thiên_can": 50,
    "can_tàng": { "quý": 50 }
  },
  "quý sửu": {
    "thiên_can": -40,
    "can_tàng": { "kỷ": -30 }
  },
  "giáp dần": {
    "thiên_can": 50,
    "can_tàng": { "giáp": 50 }
  },
  "ất mão": {
    "thiên_can": 50,
    "can_tàng": { "ất": 50 }
  },
  "bính thìn": {
    "thiên_can": -30,
    "can_tàng": { "mậu": 30 }
  },
  "đinh tỵ": {
    "thiên_can": 50,
    "can_tàng": { "bính": 50 }
  },
  "mậu ngọ": {
    "thiên_can": 30,
    "can_tàng": { "đinh": -30 }
  },
  "kỷ mùi": {
    "thiên_can": 50,
    "can_tàng": { "kỷ": 50 }
  },
  "canh thân": {
    "thiên_can": 50,
    "can_tàng": { "canh": 50 }
  },
  "tân dậu": {
    "thiên_can": 50,
    "can_tàng": { "tân": 50 }
  },
  "nhâm tuất": {
    "thiên_can": -50,
    "can_tàng": { "mậu": -25 }
  },
  "quý hợi": {
    "thiên_can": 50,
    "can_tàng": { "nhâm": 50 }
  }
}


export const thien_can_array = [ "giáp", "ất", "bính", "đinh", "mậu", "kỷ","canh", "tân", "nhâm", "quý"];
export const gio_sinh_array=["tý","sửu","dần","mão","ngọ","thìn","tỵ","ngọ","mùi","thân","dậu","tuất","hợi"];

export const dia_chi_array = ["thân", "dậu", "tuất", "hợi", "tý", "sửu", "dần", "mão", "thìn", "tỵ", "ngọ", "mùi"];
export const thien_can_ngay_gio={
  "giáp": "giáp",
  "kỷ": "giáp",
  "ất": "bính",
  "canh": "bính",
  "bính": "mậu",
  "tân": "mậu",
  "đinh": "canh",
  "nhâm": "canh",
  "mậu": "nhâm",
  "quý": "nhâm"
  
}


export const can_chi_array = [
  "giáp tý", "ất sửu", "bính dần", "đinh mão", "mậu thìn", "kỷ tỵ",
  "canh ngọ", "tân mùi", "nhâm thân", "quý dậu", "giáp tuất", "ất hợi",
  "bính tý", "đinh sửu", "mậu dần", "kỷ mão", "canh thìn", "tân tỵ",
  "nhâm ngọ", "quý mùi", "giáp thân", "ất dậu", "bính tuất", "đinh hợi",
  "mậu tý", "kỷ sửu", "canh dần", "tân mão", "nhâm thìn", "quý tỵ",
  "giáp ngọ", "ất mùi", "bính thân", "đinh dậu", "mậu tuất", "kỷ hợi",
  "canh tý", "tân sửu", "nhâm dần", "quý mão", "giáp thìn", "ất tỵ",
  "bính ngọ", "đinh mùi", "mậu thân", "kỷ dậu", "canh tuất", "tân hợi",
  "nhâm tý", "quý sửu", "giáp dần", "ất mão", "bính thìn", "đinh tỵ",
  "mậu ngọ", "kỷ mùi", "canh thân", "tân dậu", "nhâm tuất", "quý hợi"
];

export const gio_sinh_map = new Map([
  ["23:00 - 00:59", "tý"],
  ["01:00 - 02:59", "sửu"],
  ["03:00 - 04:59", "dần"],
  ["05:00 - 06:59", "mão"],
  ["07:00 - 08:59", "thìn"],
  ["09:00 - 10:59", "tỵ"],
  ["11:00 - 12:59", "ngọ"],
  ["13:00 - 14:59", "mùi"],
  ["15:00 - 16:59", "thân"],
  ["17:00 - 18:59", "dậu"],
  ["19:00 - 20:59", "tuất"],
  ["21:00 - 22:59", "hợi"]
]);


export const calculate_can_chi_gio=`Địa Chi của giờ sẽ được chia theo bảng sau.

Địa Chi giờ
Giờ theo Dương Lịch
Tý
23 giờ (Hôm trước) - 0 giờ 59 phút
Sửu
1 giờ - 2 giờ 59 phút
Dần
3 giờ - 4 giờ 59 phút
Mão
5 giờ - 6 giờ 59 phút
Thìn
7 giờ - 8 giờ 59 phút
Tỵ
9 giờ - 10 giờ 59 phút
Ngọ
11 giờ - 12 giờ 59 phút
Mùi
13 giờ - 14 giờ 59 phút
Thân
15 giờ - 16 giờ 59 phút
Dậu
17 giờ - 18 giờ 59 phút
Tuất
19 giờ - 20 giờ 59 phút
Hợi
21 giờ - 22 giờ 59 phút


Thiên Can của giờ sẽ được tính dựa vào Thiên Can của ngày.
Thiên Can Ngày
Giáp, Kỷ
Ất, Canh
Bính, Tân
Đinh, Nhâm
Mậu, Quý
Thiên Can giờ Tý
Giáp
Bính
Mậu
Canh
Nhâm

Thiên Can các giờ tiếp theo sẽ tịnh tiến theo đúng thứ tự của vòng Thiên 	Can: Giáp, Ất, Bính, Đinh, Mậu, Kỷ, Canh, Tân, Nhâm, Quý.
`

export const can_duong=["giáp", "bính", "mậu", "canh", "nhâm"];
export const can_am=["ất", "đinh", "kỷ", "tân", "quý"];

export const can_tang_array={
  "tý": ["quý"],
  "sửu": ["kỷ", "quý", "tân"],
  "dần": ["giáp", "bính", "mậu"],
  "mão": ["ất"],
  "thìn": ["mậu", "ất", "quý"],
  "tỵ": ["bính", "canh", "mậu"],
  "ngọ": ["đinh", "kỷ"],
  "mùi": ["kỷ", "đinh", "ất"],
  "thân": ["canh", "nhâm", "mậu"],
  "dậu": ["tân"],
  "tuất": ["mậu", "tân", "đinh"],
  "hợi": ["nhâm", "giáp"]
}

export const nhat_chu={
 
    "giáp": {
      "thiên_can": {
        "giáp": "tỷ",
        "ất": "kiếp",
        "bính": "thực",
        "đinh": "thương",
        "mậu": "thiên tài",
        "kỷ": "chính tài",
        "canh": "sát",
        "tân": "quan",
        "nhâm": "thiên ấn",
        "quý": "chính ấn"
      }
    },
    "ất": {
      "thiên_can": {
        "giáp": "kiếp",
        "ất": "tỷ",
        "bính": "thương",
        "đinh": "thực",
        "mậu": "chính tài",
        "kỷ": "thiên tài",
        "canh": "quan",
        "tân": "sát",
        "nhâm": "chính ấn",
        "quý": "thiên ấn"
      }
    },
    "bính": {
      "thiên_can": {
        "giáp": "thiên ấn",
        "ất": "chính ấn",
        "bính": "tỷ",
        "đinh": "kiếp",
        "mậu": "thực",
        "kỷ": "thương",
        "canh": "thiên tài",
        "tân": "chính tài",
        "nhâm": "sát",
        "quý": "quan"
      }
    },
    "đinh": {
      "thiên_can": {
        "giáp": "chính ấn",
        "ất": "thiên ấn",
        "bính": "kiếp",
        "đinh": "tỷ",
        "mậu": "thương",
        "kỷ": "thực",
        "canh": "chính tài",
        "tân": "thiên tài",
        "nhâm": "quan",
        "quý": "sát"
      }
    },
    "mậu": {
      "thiên_can": {
        "giáp": "sát",
        "ất": "quan",
        "bính": "thiên ấn",
        "đinh": "chính ấn",
        "mậu": "tỷ",
        "kỷ": "kiếp",
        "canh": "thực",
        "tân": "thương",
        "nhâm": "thiên tài",
        "quý": "chính tài"
      }
    },
    "kỷ": {
      "thiên_can": {
        "giáp": "quan",
        "ất": "sát",
        "bính": "chính ấn",
        "đinh": "thiên ấn",
        "mậu": "kiếp",
        "kỷ": "tỷ",
        "canh": "thương",
        "tân": "thực",
        "nhâm": "chính tài",
        "quý": "thiên tài"
      }
    },
    "canh": {
      "thiên_can": {
        "giáp": "thiên tài",
        "ất": "chính tài",
        "bính": "sát",
        "đinh": "quan",
        "mậu": "thiên ấn",
        "kỷ": "chính ấn",
        "canh": "tỷ",
        "tân": "kiếp",
        "nhâm": "thực",
        "quý": "thương"
      }
    },
    "tân": {
      "thiên_can": {
        "giáp": "chính tài",
        "ất": "thiên tài",
        "bính": "quan",
        "đinh": "sát",
        "mậu": "chính ấn",
        "kỷ": "thiên ấn",
        "canh": "kiếp",
        "tân": "tỷ",
        "nhâm": "thương",
        "quý": "thực"
      }
    },
    "nhâm": {
      "thiên_can": {
        "giáp": "thực",
        "ất": "thương",
        "bính": "thiên tài",
        "đinh": "chính tài",
        "mậu": "sát",
        "kỷ": "quan",
        "canh": "thiên ấn",
        "tân": "chính ấn",
        "nhâm": "tỷ",
        "quý": "kiếp"
      }
    },
    "quý": {
      "thiên_can": {
        "giáp": "thương",
        "ất": "thực",
        "bính": "chính tài",
        "đinh": "thiên tài",
        "mậu": "quan",
        "kỷ": "sát",
        "canh": "chính ấn",
        "tân": "thiên ấn",
        "nhâm": "kiếp",
        "quý": "tỷ"
      }
    }
  
}

export const menh=["kim","thủy","hỏa","thổ","mộc"]

export const can_menh=[["giáp","ất"],["bính","đinh"],["mậu","kỷ"],["canh","tân"],["nhâm","quý"]]

export const chi_menh=[["tý","sửu","ngọ","mùi"],["dần","mão","thân","dậu"],["thìn","tỵ","tuất","hợi"]]

export const menh_sinh=["mộc","hỏa","thổ","kim","thủy"];




export const menh_khac=["mộc","thổ","thủy","hỏa","kim"];
 
